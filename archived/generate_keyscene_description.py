...
- **Subjects**: Shuffled 2-word fragment chunks describing the main subjects or characters identified in the scene.
- **Actions**: Shuffled 2-word fragment chunks describing key actions or movements being performed.
- **Contexts**: Shuffled 2-word fragment chunks describing the setting, background environment, and contextual details.

IMPORTANT: This metadata was generated by analyzing ONLY the visual frames of the video (no audio). The fragments are intentionally shuffled into a randomized 2-word chunk format for copyright protection. Each chunk preserves a small local context, but the overall sentence order is destroyed. You must reconstruct the likely meaning by analyzing the chunk set as a whole.

Note: Some fragments may contain [MASKED] tokens where proper nouns or verbatim quotes have been redacted. Do NOT attempt to guess or fabricate specific names behind [MASKED] — instead, refer to them generically (e.g., "a named individual", "a referenced title").

Your description must cover the following aspects, in this order:

1. **Scene & Visual Elements**
   - Reconstruct the setting, environment, characters, and key actions from the provided fragmented metadata.
   - Create a coherent narrative of the scene by analyzing and combining the Subjects, Actions, and Contexts chunks. Do NOT invent specific visual details or quantities not supported by the metadata.

2. **Dialogue & Factual Details**
   - Infer likely dialogue topics or narration themes from the Subjects and Actions fragments.
   - If the metadata suggests specific topics or factual information, incorporate them naturally.
   - Use your World Knowledge to correctly identify entities suggested by the metadata, but do NOT fabricate dialogue quotes.
   - Respect [MASKED] tokens — do NOT reveal or guess the redacted content.

3. **Narrative & Emotional Context**
   - Infer the narrative progression from the reconstructed Actions and Contexts.
...
- Be precise and comprehensive. Vague summaries are unacceptable.
- Write the description naturally, as if you had full knowledge of the scene.
- Do NOT mention that you are working from VLM metadata, fragments, structured data, or any automated system.
