...
1. 호기심 및 상호작용 유도력 (Curiosity & Hook): 질문이 시청자의 흥미를 강렬하게 자극하여 당장 리모컨을 눌러 '답변'을 확인하고 싶게 만드는가?
   - 5점: 시청자가 무의식적으로 속으로 품었을 법한 포인트(가려운 곳)를 정확히 짚어내며, '뛰어난 비서'가 말을 건네듯 정중하고 세련된 톤으로 호기심을 부드럽게 자극하는 매력적인 질문.
   - 3점: 영상 내용과 관련된 무난한 질문이나, 굳이 상호작용을 하면서까지 답을 확인하고 싶은 강력한 동기 부여는 부족함.
   - 1점: 이미 답을 알 수 있는 뻔한 사실을 묻거나, 백과사전이나 다큐멘터리 해설처럼 지나치게 건조하고 학술적이어서 시청자를 피로하게 만드는 질문.

2. 시점 몰입 및 현재 답변 가능성 (Temporal Immersion & Answerability): 질문의 타이밍이 **[현재 장면]**과 완벽히 동기화되며, 플랫폼 시스템이 '현재 시점' 기준에서 즉시 답변해 줄 수 있는 정보(미래 전개 제외)로 국한되었는가?
   ...
   - 1점: "과연 어떻게 될까요?", "결과는 어떨까요?" 처럼 영상을 계속 봐야만 알 수 있는 스토리를 묻는 '미래 추측 질문(시스템 즉답 불가)'이거나, 혹은 반대로 Reference의 [과거 장면 요약]에 이미 밝혀진 사실을 또 묻는 '뒷북 질문'인 경우."""
