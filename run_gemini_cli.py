...
def init_gemini_client(gcp_project_id):
    # vertexai.init(project=gcp_project_id, location="asia-northeast3")   # Seoul
    vertexai.init(project=gcp_project_id, location="global")   # Global
    return None
...
def configure_system_prompt(mode="full"):
    if mode == "full":
        ...
        **외부 자료 검색 금지**: 외부 자료를 검색하여 답변하지 말고, 제공된 정보만 사용하여 답변하세요.
        """
    elif mode == "nodesc":
        system_prompt = """
        첨부한 파일은 긴 비디오 영상을 15초 단위의 세그먼트로 나누어 데이터를 추출한 JSONL (JSON Lines) 파일입니다. 이 영상 데이터를 전체적으로 이해한 후, 마지막에 주어지는 **사용자 질문**에 대해 가장 정확하고 종합적인 답변을 한국어로 작성해 주세요.
        ...
        **외부 자료 검색 금지**: 외부 자료를 검색하여 답변하지 말고, 제공된 정보만 사용하여 답변하세요.
        ...
        system_prompt = "외부 정보를 절대 검색하지 말고, 제공된 영상 정보만을 사용하여 사용자 질문에 답변하세요."
...
def init_generation_model(mode="full", model_name='gemini-2.5-flash'):
    system_prompt = configure_system_prompt(mode)
    if system_prompt:
...
def evaluate_answer(judge_model, video_part, user_prompt, generated_answer, gt_json_part=None):
    user_content = f"[사용자 질문]\n{user_prompt}\n\n[평가 대상 답변]\n{generated_answer}"
    if gt_json_part:
        contents = [video_part, gt_json_part, user_content]
    else:
        contents = [video_part, user_content]
    
    response = judge_model.generate_content(contents)
