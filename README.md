...
flowchart TD
    ...
    subgraph STEP2["Voice Hint 생성"]
        direction TB
        VH_SCRIPT["**generate_voice_hint.py**"]
        VH_OUT["**voice_hint.jsonl**<br/>Voice Hint + Detailed Summary"]
    end

    subgraph STEP3["Voice Hint Scoring"]
        direction TB
        QJ_SCRIPT["**judge_voice_hint.py**<br/>(Detailed Summary 기반)"]
...
| A-2 | `generate_voice_hint.py` | VH: Desc JSONL</br>Summary: Video+Ref JSONL | `voice_hint.jsonl`, `vh_summary.jsonl` | Flash (VH), Pro (Summary) |
| A-3 | `judge_voice_hint.py` | VH, Summary | `voice_hint_scores.jsonl` | Pro |
...
- **Voice Hint** (현재 장면 중심): 현재 장면의 시각적 요소, 대사, 행동에만 기반한 즉각적인 질문. Detailed Summary와 함께 생성되어 텍스트 기반 품질 평가(Scoring)를 받습니다.
...
- **Desc JSONL** (`_Desc.jsonl`): VLM이 생성한 종합 서술(description 필드). 시각, 대사, 자막, 환경음을 하나의 텍스트로 통합. → 평가 대상 모드(desc), Voice Hint 생성, User Query 생성에 사용.
...
├── generate_voice_hint.py      # Keypoint 입력 → Voice Hint & Summary 생성
├── generate_user_query.py        # Keypoint 입력 → User Query 생성
├── judge_voice_hint.py         # Voice Hint 품질 평가 (Scoring)
...
python main.py --skip-voice-hint-gen --skip-query-judge --skip-user-query-gen --skip-response --skip-judge
...
python main.py --skip-keypoint --skip-voice-hint-gen --skip-query-judge

# Query 생성 건너뛰고 이후 답변/평가만
python main.py --skip-keypoint --skip-voice-hint-gen --skip-query-judge --skip-user-query-gen
```
...
# A-2: Voice Hint 생성 (Keypoint 입력)
python generate_voice_hint.py

# A-3: Voice Hint 품질 Judge
