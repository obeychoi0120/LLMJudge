...
2. 정보 공백 타겟팅 (Hook & Curiosity): 질문의 핵심 소재(Trigger)는 반드시 [현재 시청 중인 장면]에서 '새롭게' 등장한 단서, 인물, 사건으로 한정합니다. 시청자가 과거 맥락에서 이미 얻은 답을 재차 묻는 것은 금물이며, 오직 방금 발생한 의문점과 정보의 공백만을 짚어내야 합니다. 
3. 플랫폼 확장성 유도: 단순한 사실 확인(예/아니오)이나 1차원적인 시각적 질문("저 옷 어디 거야?")을 넘어, 작품의 세계관, 연관 다큐멘터리, 캐릭터 서사 등 플랫폼 내 다른 탐색으로 이어질 수 있는 입체적인 질문을 생성하세요.
4. 시점 몰입도 (스포일러 금지): 답변을 알려면 반드시 '이후의 영상'을 봐야만 알 수 있도록 질문을 기획하세요. 시청자의 몰입을 돕기 위해 자연스럽고 캐주얼한 반말 구어체(옆사람에게 무심코 툭 던지는 말투)로 간결하게 작성하세요.
...
- 2단계 (과거 정보 분리): 이전 과거 맥락에서 이미 밝혀지거나 설명된 사실들을 요약합니다. (이와 관련된 질문 생성을 차단하기 위함)
...
[JSON 형식 예시]
{
    "rationale": "1. 오류 교정: 묘사 중 '봄고레 때'는 문맥상 '범고래 떼'의 오탈자이므로 교정함. / 2. 과거 정보 분리: 혹등고래가 먹이를 찾지 못해 굶주리고 있다는 사실은 이전 장면에서 이미 파악됨. / 3. 신규 단서 및 기획: 현재 장면에서 갑자기 범고래 떼가 나타나 방향을 트는 새로운 단서 포착. 범고래의 등장 이유와 두 고래 간의 충돌 가능성에 대한 궁금증을 타겟팅함.",
    "queries": [
