...
- description: 소형 AI가 해당 Scene의 시각적 상황, 인물 행동, 대사, 자막 등을 종합해 자동 생성한 묘사 (오탈자 및 환각이 포함될 수 있음)

[질문 생성 핵심 전략]
1. VLM 노이즈 자체 교정 및 스무딩 (최우선 순위): 제공된 `description`은 소형 AI의 결과물이므로 음성 인식 오류, 자막 OCR 오탈자(예: '세프'->'셰프', '봄고레'->'범고래'), 띄어쓰기 오류, 엉뚱한 고유명사 등 환각(Hallucination)이나 노이즈가 포함될 수 있습니다. 텍스트를 기계적으로 맹신하지 말고, 앞뒤 맥락과 당신의 '풍부한 일반 상식'을 바탕으로 명백한 오류를 올바른 정보로 필터링 및 교정(Smoothing)한 후 질문을 기획하세요. 화면에 직접 노출될 최종 질문에는 절대 오탈자나 어색한 단어가 포함되어서는 안 됩니다.
2. 정보 공백 타겟팅 (Hook & Curiosity): 시청자가 [1. 과거 맥락]을 통해 이미 알고 있는 사실은 절대 묻지 마세요 (뒷북 금지). 과거 정보와 [2. 현재 장면]이 연결되며 새롭게 발생하는 '의문점'과 '정보의 공백'을 날카롭게 짚어내세요.
...
4. 시점 몰입도 (스포일러 금지): 질문의 트리거는 철저히 '현재 장면'에서 파생되어야 하며, 미래 전개를 암시하거나 예측하는 질문은 몰입을 깨므로 절대 금지합니다.
