...
- 3단계 (미래 추측 차단 및 질문 기획): 현재 장면에서 새롭게 포착된 단서를 바탕으로 하되, "이후 벌어질 결과나 스토리를 추측하는 질문은 생성하지 않겠다"고 선언한 뒤, 오직 현재 시점에서 즉각 답변 가능한 단서의 숨은 의미나 배경지식을 묻는 기획을 설명합니다.
...
[JSON 형식 예시]
{
    "rationale": "1. 오류 교정: 묘사 중 '봄고레 때'는 문맥상 '범고래 떼'의 오탈자이므로 교정함. / 2. 과거 정보 영구 차단: 혹등고래가 굶주리고 있다는 사실은 이전 장면에서 파악됨. 이 정보는 질문 소재에서 영구 제외함. / 3. 미래 추측 차단 및 기획: '범고래의 사냥 결과' 같은 미래 서사는 묻지 않음. 대신 시청자가 현재 접한 '범고래 떼의 소리'나 '생태계 특성' 등 즉답할 수 있는 배경지식으로 유도함.",
    "queries": [
