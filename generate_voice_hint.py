...
이 두 정보의 흐름을 파악하여, TV 화면의 버튼을 눌러 답을 확인하고 싶게 만드는 매력적인 질문 **2개**를 생성하세요.
...
[질문 생성 핵심 전략]
1. 시점 몰입 및 현재 답변 가능성 고려 (미래 추측 & 과거 뒷북 방지): 질문은 오직 현재 즉시 정보를 제공할 수 있는 [현재 장면] 속 사물, 인물의 정체, 배경지식, 상황의 숨은 의미로 한정해야 합니다. **앞으로 전개될 스토리나 미래의 결과(예: "과연 어떻게 될까요?", "어떤 평가를 받을까요?")를 묻는 질문은 시스템이 당장 답할 수 없어 철저히 0점 처리됩니다.** 또한, **[1. 과거 장면 요약]에서 이미 설명되었거나 영상을 시청하지 않아도 일반적인 상식으로 유추할 수 있는 뻔한 사실을 묻는 '뒷북 질문' 역시 무조건 0점 처리됩니다.**
2. 호기심 및 상호작용 유도력 (Hook): 위 조건을 만족하는 범위 내에서, 질문의 핵심 소재(Trigger)는 반드시 [2. 현재 장면 묘사]에서 새롭게 발생한 의문점과 정보의 공백(미지수)만을 예리하게 짚어내야 합니다. 어조(Tone)는 KSS에 묘사된 씬의 분위기를 절대 깨지 않도록 '해당 장르/컨텐츠의 전문 평론가'가 말을 건네듯 정중하고 세련된 존댓말로 작성하여 시청자의 흥미를 강렬하게 자극하세요.

[사고 과정 (Chain-of-Thought) 가이드]
질문을 생성하기 전에 `rationale` 필드에 반드시 다음 3단계를 순서대로 작성하여 논리적으로 사고하세요.
- 1단계 (과거 정보 차단): [1. 과거 장면 요약]에서 이미 파악된 사실이나 일반 상식을 요약한 뒤, "이 정보들은 질문 소재에서 제외한다"라고 선언하세요.
- 2단계 (미래 추측 차단): "결과나 스토리 전개를 묻는 미래 지향적 질문을 차단하겠다"라고 선언하세요.
- 3단계 (Hook 및 질문 기획): [2. 현재 장면 묘사]에 등장한 새 단서에 집중하여, 현재 시점에서 답변 가능하면서도 흥미로운 정보(정체, 배경지식, 숨은 의미 등)를 묻는 기획을 설명합니다.
    ...
    def generate_voice_hints(mode):
        ...
        if mode == "kss":
            ...
            contents = [
                ...
                kss_summary_text, 
                "--- 요청 사항 ---",
                "위의 [KeyScene Summary]를 읽고, 시스템 프롬프트의 [사고 과정 가이드] 지침을 철저히 준수하여 질문 **2개**를 JSON 형식으로 생성하세요. 오직 방금 본 [2. 현재 장면 묘사]에 등장한 '새로운' 단서에 호기심을 집중하세요."
            ]
